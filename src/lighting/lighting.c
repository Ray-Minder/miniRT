...
/*                                                   +#+                      */
/*   Created: 2025/04/30 12:12:33 by eandela       #+#    #+#                 */
/*   Updated: 2025/04/30 12:12:34 by eandela       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
t_color	lighting(t_scene *scene, t_comps *comps)
{
	...
	ambient_contribution = ambient_lighting(scene->ambient_light,
			comps->object->color);
	diffuse_contribution = diffuse_lighting(scene->light, comps->normalv,
			comps->point, comps->object->color);
...
t_color	ambient_lighting(t_ambient_light ambient_light, t_color object_color)
{
	...
	ambient = multiply_color_by_scalar(ambient_light.color,
	...
	t_tuple position, t_color object_color)
{
	...
	light_direction = normalize_tuple(subtract_tuples(light.position,
				position));
	...
	{
		diffuse = multiply_color_by_scalar(object_color,
